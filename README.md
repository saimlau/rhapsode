...
subcollections as their own "Parent / Child" playlists. Install the
[latest `rhapsode.xpi`](https://github.com/saimlau/rhapsode/releases/latest)
via Tools → Plugins → ⚙ → Install Plugin From File (auto-updates from
releases), or for development use `zotero-plugin/dev-install.sh` while
Zotero is closed. Works on Zotero 7–9.
...
## 🗣️ Kokoro as a system voice (Speech Dispatcher)
